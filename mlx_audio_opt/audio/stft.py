...
def get_spectrogram(
    ...
    real = np.array(mx.real(complex_spectrogram), dtype=np.float32)
    imag = np.array(mx.imag(complex_spectrogram), dtype=np.float32)
    complex_spectrogram_np = real + 1j * imag
    phase = np.angle(complex_spectrogram_np).T
